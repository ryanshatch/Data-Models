    ...
    if (root == nullptr) {
        ...
        while (true) {
            if (bid.bidId < current->bid.bidId) {
                ...
                // If the bid ID is larger or equal, go to the right subtree
...
Node* BinarySearchTree::findMin() {
    ...
    // Traverse the left sub-tree until you reach the node with the smallest value
...
void BinarySearchTree::inOrder(Node* node) {
    ...
    cout << node->bid.bidId << ": " << node->bid.title << " | " << node->bid.amount << " | " << node->bid.fund << endl; // output the bid details.
